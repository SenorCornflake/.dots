...
if len(args_excluding_options) == 3:
	...
	if os.path.exists(os.path.expanduser("~/.config/kitty/colors/{}.conf").format(colorscheme)):
		exconman_settings["kitty.colorscheme"] = "~/.config/kitty/colors/{}.conf".format(colorscheme)
	else:
		kitty()
else:

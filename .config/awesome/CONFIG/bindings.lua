...
-- CLIENT BUTTONS --
--------------------
bindings.client_buttons = gears.table.join(
	awful.button(
		{ },
		1,
		function (c)
	...
	awful.key(
		...
		function()
			hotkeys_popup.widget.new({
				...
				description_font = theme.alpha_bar_font,
				modifiers_fg = theme.alpha_bar_label_fg,
				label_bg = theme.alpha_bar_occupiedFagFg,
				label_fg = theme.alpha_bar_bg,
				border_width = theme.general_borderWidth,
				border_color = theme.general_borderFocused,
			}):show_help()
		end,
		{
...
bindings.client_keys = gears.table.join(
	awful.key(
		{ super },
		"w",
		function(c)
